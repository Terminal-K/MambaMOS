...
@DATASETS.register_module()
class DefaultMultiScansDataset(Dataset):
    def __init__(
        self,
        split="train",
        data_root="data",
        ...
        super(DefaultMultiScansDataset, self).__init__()
        self.data_root = data_root
        self.split = split
        ...
        if test_mode:
            self.scale2kitti = (
                TRANSFORMS.build(self.test_cfg.scale2kitti)
                if self.test_cfg.scale2kitti is not None
                else None
            )
            self.test_voxelize = (
                TRANSFORMS.build(self.test_cfg.voxelize)
    ...
    def prepare_test_data(self, idx):
        ...
        data_dict = self.transform(data_dict)
        result_dict = dict(
            segment=data_dict.pop("segment"), name=self.get_data_name(idx), tn=data_dict["tn"],
        )
        if "origin_segment" in data_dict:
            assert "inverse" in data_dict
            result_dict["origin_segment"] = data_dict.pop("origin_segment")
